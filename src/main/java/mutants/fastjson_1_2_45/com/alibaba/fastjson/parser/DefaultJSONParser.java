...
public class DefaultJSONParser implements Closeable {
    ...
    public final Object parseObject(final Map object, Object fieldName) {
        ...
        if (lexer.token() == com.alibaba.fastjson.parser.JSONToken.LITERAL_STRING && lexer.stringVal().length() == 0) {
            lexer.nextToken();
            return object;
        }
            ...
            for (;;) {
                ...
                if (ch == '"') {
                    ...
                    try {
                        ...
                        if (lexer.isEnabled(com.alibaba.fastjson.parser.Feature.NonStringKeyAsString)) {
                            key = key.toString();
                        }
                ...
                if (key == JSON.DEFAULT_TYPE_KEY
                        && !lexer.isEnabled(com.alibaba.fastjson.parser.Feature.DisableSpecialKeyDetect)) {
                    String typeName = lexer.scanSymbol(symbolTable, '"');

                    if (lexer.isEnabled(com.alibaba.fastjson.parser.Feature.IgnoreAutoType)) {
                        continue;
                    }
...
//                    }
                    ...
                    Map input;
                    if (lexer.isEnabled(com.alibaba.fastjson.parser.Feature.CustomMapDeserializer)) {
                        ...
                        input = new JSONObject(lexer.isEnabled(com.alibaba.fastjson.parser.Feature.OrderedField));
                    }
    ...
    public void parseArray(Type type, Collection array, Object fieldName) {
        int token = lexer.token();
        if (token == com.alibaba.fastjson.parser.JSONToken.SET || token == com.alibaba.fastjson.parser.JSONToken.TREE_SET) {
            lexer.nextToken();
            token = lexer.token();
        }

        if (token != com.alibaba.fastjson.parser.JSONToken.LBRACKET) {
            throw new JSONException("exepct '[', but " + com.alibaba.fastjson.parser.JSONToken.name(token) + ", " + lexer.info());
        }
...
//            }
    ...
    public void close() {
        ...
        try {
            if (lexer.isEnabled(com.alibaba.fastjson.parser.Feature.AutoCloseSource)) {
                if (lexer.token() != com.alibaba.fastjson.parser.JSONToken.EOF) {
                    throw new JSONException("not close json text, token : " + com.alibaba.fastjson.parser.JSONToken.name(lexer.token()));
                }
            }
        } finally {
            lexer.close();
    ...
    public Object resolveReference(String ref) {
        ...
        for (int i = 0; i < contextArray.length && i < contextArrayIndex; i++) {
            com.alibaba.fastjson.parser.ParseContext context = contextArray[i];
    ...
    public static class ResolveTask {
        ...
        public ResolveTask(com.alibaba.fastjson.parser.ParseContext context, String referenceValue){
            this.context = context;
    ...
    public Object parse(PropertyProcessable object, Object fieldName) {
        if (lexer.token() != com.alibaba.fastjson.parser.JSONToken.LBRACE) {
        ...
        try {
            for (int i = 0;;++i) {
                ...
                if (tok == com.alibaba.fastjson.parser.JSONToken.EOF || tok == com.alibaba.fastjson.parser.JSONToken.RBRACKET) {
                    return object;
                }
